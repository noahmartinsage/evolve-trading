...
> **v11 追加（2026-09-17 第十一轮，只增补不重写）**：
> 1. 新增 **L6.20 让一个已有助手「知道自己的系统」：三态事实 + 能力注册表** ——
>    用户点名它答不上来两件事：「Agent 舰队里哪个策略盈利最高」「系统里还有进化实验室」。
>    真因是**没有数据却答得像有**（舰队页的收益是页面演示常量；流水线统计只有笔数与回撤、**没有金额**）。
>    ⇒ 每个事实带 `live` / `demo` / `unavailable` 三态 + 证据来源，
>    **`source !== 'live'` 的必须在回话里说出来**。
> 2. 新增**机制化**做法：「不允许任何语音专用实现」这条红线，从纪律变成**必填字段** ——
>    每个能力必须声明它复用哪条既有路径（`reuses`），空着就注册失败并由门禁报红。
>    拦在"填写那一刻"，比事后审计有效。
> 3. 门禁总数**不变（29 道）**，断言数上升：`test:voice` 14 → **15 场景**（+S14 系统实况与工具注册表）。
> 4. ★★ 新增**两个坑**（都属于"负向断言被邻居顶替"，是第一轮变异实测逃逸出来的）：
>    ① **一个坏输入同时犯两个错** ⇒ 有一类检查**从来没被喂过**，
>       整块删掉也不报红（又是一段"不可能失败的检查"）。
>       ⇒ 每个坏输入只犯一个错；断言必须把问题**归属到具体 id**，
>       而不是"某条问题里出现了某个词"。
>    ② **"随便哪一处说了" 顶替 "每一处都说了"** ⇒ 把 `speakSituation()` 里
>       "系统里没有按策略的盈亏数据"整句删掉，断言仍然满足 ——
>       **恰恰最该被防的"沉默失效"，它自己是静的。**
>       ⇒ 改成一张**逐份文案的披露欠条**，谁欠哪句就由谁报红；并补反向。
> 5. 新增两条**判据级**提问（编号 20 / 21，已并入 `DEV_PROGRESS §7`）：
>    **「我这条负向断言，有没有一个*只有它*会命中的输入？」**
>    **「我说『缺数据必须被说出来』，是*每一处*都说了，还是*随便哪一处*说了？」**
> **v12 追加（2026-09-18 第十二轮，只增补不重写）**：
> 1. 新增 **L6.21 因子批量生产线：能被拒绝，才算一条产线** —— 用户要求"AI 量化因子批量生产达商用级"。
>    ★ 最值钱的一条不是抄到了什么，而是**查出上游最核心的统计量在本项目里不成立**：
>    上游的横截面 Rank IC 要求横截面足够宽，而本项目标的池只有 BTC/ETH（宽度 2）——
>    算得出 IC、ICIR，数值**看着完全正常**，只是它测的东西不存在（判据 5 的最难形态）。
>    ⇒ 改时间序列 Rank IC。门禁 **29 → 30**（+`test:factors` 10 场景）。
> 2. 新增一条**形态级**判据：**枚举本身就是判据。** 与原始值**秩等价**的变换（`z`/`rank`/`tanh`）
>    不靠"事后加一层过滤器"排除，而是**根本不放进 `FACTOR_TRANSFORMS`**。
>    实测：40 个候选里 15 个被秩相关判重拒掉，而**没有一个是同名重复**（同名早被 slug 拦了）
>    ⇒ 这 15 个在"名字串去重"的口径下本来会全部混进台账。
> 3. **顺手修掉一处既有缺陷**（与 F-47 同型）：提案引擎在**合成 GBM** 上选候选、过拟合门拿**真实历史**判 ——
>    "选"与"判"吃两批数据。门禁的数字是对的，但它判的候选从来不是真实数据上选出来的。
>    ⇒ 统一到 `loadEvidence()`，并加断言比 `dataHash`。
> 4. 新增一条**问法**（判据 5 的镜像，适用面比因子广）：
>    ★ **「这个状态不是「代码里不可能发生」，而是「在我这个夹具里不可能发生」？」**
>    实测：F6 用"全部门都打开"的常量去验去冗余 ⇒ 被测闸也被开到了极端，
>    该断言**在它自己的场景里永远不可能触发**。⇒ 场景常量要分"只开别的门"与"全开"。
> **v13 追加（2026-09-18~19 第十四~十六轮，只增补不重写）**：
> 1. 新增 **L6.22 Agent 舰队：不许有孤岛要变成机制** —— 用户要求"Agent 舰队需要真实干活跑实测的"。
>    ★ 最值钱的一条不是"能派活了"，而是**写烟测时抓出一处真代码缺陷**：断链判定原本按"实际发出的主题"算，
>    于是上游声明要发却静默不发时，这次任务被降级成"没有上下游"而**报成功**（判据 11 的同族）。
>    ⇒ 改按"上游**声明的** `emits`"判定，断链整体失败。门禁 **31 → 32**（+`test:fleet` 22 场景）。
> 2. 因子线定案：**每笔毛边际 ≈ 0.51 bps vs 每笔往返成本 ≈ 6.48 bps**，
>    而"空间不够大 / 不能做空 / 阈值不对 / 该降换手 / 该换因子族"五条假设**逐条被实测证伪**。
>    ★ 换掉的那个判据（拿最差折毛收益分流"成本问题 vs 方向不成立"）本身就是错的：
>    同一批数据上它判 **1/14**，换平均折判 **11/14** —— 判据选错，结论会把下一步投到错的方向。
> 3. 失败三态（见 L6.22.4）：**"全都失败了"必须能指出该动哪一处** —— 额度 / 名单 / 开关，三者下一步相反。
> 4. 新增三条**判据级**提问（编号见 `DEV_PROGRESS §7` 与 `MEMORY.md`）：
>    **「我用哪个统计量来把这两件事分开？」**
>    **「这个失败，我分得清『能力没有』与『额度用完了』吗？」**
>    **「这条负向断言选的那个词，会不会在本分支里以否定形式出现？」**
> 5. 新增一条**报表**要求：**"没抓住"与"被别的场景抓住"必须能区分** ——
>    变异测试的预期场景写成**数组**，否则真被抓到也会被读成逃逸。
> 6. ★ 新增一类**检查器盲区**（比因子本身更通用）：`_v9_mdtable.mjs`（文档表格校验器）的扫表循环是
>    "行以 `|` 开头**且**以 `|` 结尾"，一旦某行不满足就**退出扫表、之后的内容再也不看** ⇒
>    "单元格里被插了裸换行 / 行尾漏了闭合竖线"这类错位，它永远报"0 处"。
>    本轮实测：自己在 §7 只改 2 行，造出的 **3 处错位全被"0 处"藏住**。
>    已补"扫完再往前看一行"的检测，并**正反两跑**验证（真实文档 126 张表 0 处 · 坏夹具 3 张表 2 处）。
>    ★ 通用问法：**「这个校验器扫到哪就停了？」** —— 报 0 不等于没有错。
> 7. 新增两条**文档编辑**事实：本仓库两份主文档**行尾不一致**（`DEV_PROGRESS` 是 LF、
>    `DEV-PROMPT-KIT` 是 CRLF）⇒ 用脚本批量替换时多行锚点会**静默不命中**（按目标文件归一即可）；
>    以及 **markdown 表格单元格里不能有裸换行**，插入表格行时不要把行拆成多个数组元素。
...
- ✅ 30 道 CI 门禁（`npm run ci`；清单与实跑断言数以 **Part I** 为唯一出处）
...
```json
{
  ...
  "ci": "npm run lint && npm run typecheck && … 共 30 道，详见 Part I"
}
```
...
├── .github/workflows/ci.yml    # ⚠️ 仅 5 道门禁，与本地 npm run ci（30 道，见 Part I）不一致
...
## D3 · 编排服务 API 契约（实测 105 个路径）

> ★ **2026-09-18 第十三轮复核（现算，非沿用）**：对 `server/index.ts` 与 `server/ledgerServer.ts` 同时匹配
> `url.pathname === '…'` 与 `url.pathname.startsWith('…')`（后者 3 个前缀路由
> `/orders/` `/promotions/` `/llm/providers/`），**去重后 92 个唯一路径**。
> 组成：`index.ts` 87 个 `===` + 3 个 `startsWith`，`ledgerServer.ts` 5 个 `===`（两侧无重叠）。
> 演变轨迹：89（第七轮）→ 91（第十二轮 +`/factors/index`、`/factors/generate`）→ 92（第十三轮 +`/factors/strategies`）→ **103**（2026-09-20 第二十二轮 +`/orders/precheck`）→ **104**（第二十三轮 +`/breadth/index`）→ **105**（第二十四轮 +`/forecast`）。
> ★ 105 的现算（`_epcount.mjs` 跑出来的，不是猜的）：`index.ts` 100 处 `===`（99 个唯一）+ 3 处 `startsWith`，`ledgerServer.ts` 5 处 `===`（5 个唯一），两文件两写法去重后 **105 个唯一路径**。
> ★ 复现这条数字请**现算**，脚本式做法见 `DEV_PROGRESS §3.24.9`（匹配两文件 × 两种写法 → 去重）。
> 历史上它漂过两次（84 → 86 → 89），每一次都是"后写的那份"过期 ——
> **同一事实存了两份，最早过期的总是后写的那份。**
...
| CI 门禁数 | "十三道" / "14 道" | **18 道**（当时值；2026-09-20 第十九轮实测 `npm run ci` = **38 步**，以 Part I 为准） | DEV_PROGRESS |
...
| GitHub CI | — | **只跑 5 道**，与本地 `npm run ci`（现 30 道）不一致 | `.github/workflows/ci.yml` |
...
| 项 | 值 |
|---|---|
| `test:mission` | **17 组（S-M1 ~ S-M17）**，全离线，归档 `artifacts/mission-latest.json` |
| `npm run ci` | **30 道**（该轮 27 → 28；`test:mission` 已进 `ci` 链，第十二轮后为 30）|
...
## L6.20 · 让一个已有助手「知道自己的系统」：三态事实 + 能力注册表（2026-09-17 第十一轮）

> 来源：§3.22。用户要求管家成为"本系统什么都知道的独立智能体"，
> 并**自己点名了两个它答不上来的问题**：
> ① "Agent 舰队里哪个策略盈利最高"；② "系统里还有进化实验室"。

### L6.20.1 先查"答不上来"是哪一种

拿"不知道"这件事，先分成三类，**分开处理**：

| 类型 | 例子 | 正确处置 |
| --- | --- | --- |
| 有真实数据，但没接进来 | 权益 / 持仓 / 心法条数 | 接进来（读既有服务函数） |
| 有数据但是**演示值** | Agent 舰队页的收益/胜率（页面自己标着"非真实 PnL"） | 接，但**必须点明"是演示"** |
| 系统里**根本没有**这个数据 | 分策略盈亏；谱系树 | **明说没有**，并给替代口径 |

★ 第 3 类最容易被做错：**补一份假数据**比"说没有"看起来更像在解决问题，
而那正是本仓库第三族 P0（答案看起来完全正确，只有真相没了）。
用户会真的拿它去决定投钱。

### L6.20.2 核心形式：每个事实带三态

```ts
type FactSource = 'live' | 'demo' | 'unavailable'
interface Fact<T> { value: T | null; source: FactSource; evidence: string }
```

四条规矩：

```text
1. `evidence` 必须能让人自己去核对（写明去哪个模块看），
   `unavailable` 时写的是"缺什么、为什么缺"。
2. 数据只从**既有服务函数**读（autopilotStatus / pipelineService.list /
   lessonStats / …），不自己拼第二份口径 —— 两个口径迟早对同一问题给出不同答案。
3. `source !== 'live'` 的事实，**回话里必须被说出来**。
   沉默会被用户读成"系统有这项" —— 而沉默失效本身是静的，不会有任何东西报红。
4. 恒 unavailable 的那一项**写成函数**（`fleetPnlVerdict()`），不写成内联字符串：
   这样"为什么答不了"可以被单独断言，将来真接上数据时**只有一个地方要改**。
```

★ `demo` 这一档存在的意义，用一个真实用例说清：
进化实验室页**确实**画了一棵谱系树（用户看得见，事实存在），但内容是页面静态常量。
⇒ 对"系统里有没有谱系数据"这个问题答案是 `unavailable`；
对"用户在页面上看到的是什么"这个问题答案是 `demo`。两件事分开说才不会误导。

### L6.20.3 把红线做成机制，而不是靠纪律

本项目有一条红线：**不允许任何"语音专用实现"**（语音单的拒绝理由必须与界面逐字相同）。
和所有"靠纪律"的规则一样，它会被忘记。做成机制：

```ts
interface VoiceTool {
  id; label; kind: 'read' | 'act'; cost;
  reuses: string        // ★ 复用哪条既有路径。空字符串 = 注册失败
  intent?: VoiceIntentName  // act 必填，且必须在 DANGEROUS 名单里
  run: (arg?: string) => VoiceToolResult | Promise<VoiceToolResult>
}
```

```text
1. `auditToolRegistry()` 在门禁里检查四条：
   id 重复 / 没有 reuses / act 没有 DANGEROUS 意图 / 没有 label。
2. 「将来有人想在语音层就地造一个」→ 他会先卡在"这一栏填什么"上，
   那一刻他就会去看既有实现在哪。**要拦的是那一刻，不是事后审计。**
3. 读取层收窄成单一入口：service 层**只**从注册表取工具，不再直接引数据层；
   工具层**只**引三个 `speak*` 出口，不引底层的 `situation()`/`fleetStandings()`。
   那些是"数据"，工具产出的必须是"话" —— 多引一个就多一条绕过统一措辞的缝。
```

### L6.20.4 「自我调用大模型改代码」的落点

这类诉求（"发现问题就自己改代码升级自己"）**不要**实现为"意图直接改源码"。

正确落点：**提案引擎 + 晋级流水线**（候选 → 过拟合门 → 纸交易观察 → 测试网实测 →
小资金 → 全量）。理由不是保守 —— 这是系统里唯一一道"改动必须留下证据"的闸，
给它开一条旁路 = 把闸拆了，而拆掉之后**没有任何东西会报红，只有钱会慢慢少**。

★ 而且这条边界要**写进自我介绍**：
"改代码这件事我不能自己拍板 —— 我只能提提案、走门禁，真正上线要你来批。"
一个只说能力不说边界的自我介绍，会让用户以为它现在就能动真钱。

### L6.20.5 ★★ 两个坑：负向断言被"邻居"顶替

本轮第一次跑变异，**10 组里逃了 2 组**，两处都是我自己刚写的断言：

```text
坑 A · 一个坏输入同时犯两个错 ⇒ 有一类检查从来没被喂过
  注入的坏工具 `act-no-intent` 同时缺 label 和 intent，
  命中的是"缺 intent"那个**前一个分支**；
  于是把"act 的意图必须在 DANGEROUS 里"整块 if 删掉，测试照样全绿。
  ⇒ 它是一段**不可能失败**的检查（本项目第 8 次同类）。
  修法两件一起做：
    ① 每个坏输入只犯**一个**错（另开一个 `no-label`）；
    ② 断言把问题**归属到具体 id**（`p.toolId === only && p.problem.includes(need)`），
       而不是"某条问题里出现了某个词" —— 后者正是被顶替的入口。
       注入用例从 4 个增到 6 个（4 类检查 → 5 类）。
  附：再给注入用例本身加一条守卫 —— 若哪天 `query_market` 被移进 DANGEROUS，
  `act-nondanger` 就不再是坏工具。assert `!isDangerous('query_market')`。

坑 B · "随便哪一处说了" 顶替了 "每一处都说了"
  「缺数据必须被说出来」那条断言只查了 `speakFleet()`。
  把 `speakSituation()` 里"系统里没有按策略的盈亏数据"整句删掉，断言仍然满足。
  ⇒ 恰恰最该被防的"沉默失效"，它自己是静的。
  修法：改成一张**逐份文案的披露欠条**（谁欠哪句，就由谁报红）：

    系统实况 ← 「没有按策略的盈亏数据」
    舰队     ← 「没有按策略的盈亏数据」+「演示/非真实」
    实验室   ← 「谱系」

  并补反向：三份文案**都不许**出现"盈利最高的是"
...
```text
不新增门禁（总数仍 29）—— 不变量全落进 `test:voice`：
  test:voice  14 → 15 场景（+S14 系统实况与工具注册表）
    · 8 条新口令逐条命中（含 2 条反例）
    · 三态双向可证（`standingsSourceOf(0)` / `(3)` 两个输入都喂）
    · 排行口径显式（`rankedBy: 'stage_then_fitness'`）且 rejected 恒垫底
    · 9 个阶段中文名零缺口（删一档就报红，报出英文原值）
    · 「盈利最高」正解为 unavailable 且理由写清（双侧极性检查）
    · 三份文案各自欠的披露逐份对账 + 都不许报出第一名
    · 自我介绍交代改代码边界；未确认不执行 / 确认后过宪法红线且不污染心法库
    · 注册表 7 工具 0 问题；注入 6 个坏工具逐条**归属**报红
    · 系统实况 10 项真实来源

破坏验证 10/10（`_mutate11.mjs`），第一遍逃逸 2 组（见 L6.20.5），修完重跑全抓住。
★ 跑 CI 必须**前台**跑（后台拿不到删除放行 ⇒ `build` 必红），理由见 L6.18.5。
```

### L6.20.7 复现这一层时的提示词（可直接粘贴）

```text
我要让一个已有的交易助手"知道自己的整套系统"，先别写代码，按顺序做：

一、把"它答不上来"的每个问题分类，分三类分别处理：
    ① 有真实数据但没接进来；② 有数据但是演示值；③ 系统里根本没有这个数据。
    对每一类说清"回话里必须出现什么"。特别是第③类 ——
    禁止为了让它"答得出来"去补一份假数据，那比说"我不知道"危险得多。

二、设计事实的表示：每个事实带 source（live / demo / unavailable）+ evidence。
    要求：
    1) evidence 能让人自己去核对（写明去哪个模块看）；
    2) 数据只能从**既有服务函数**读，不许拼第二份口径；
    3) 恒 unavailable 的那一项要**写成函数**，不是内联字符串 ——
       这样"为什么答不了"可以被单独断言。

三、把"不许在这次改动里造一条专用实现"做成**机制**而不是纪律：
    每个能力必须声明它复用哪条既有路径，空着就注册失败，并让门禁检查这件事。
    解释一下为什么"拦在填写那一刻"比"事后审计"有效。

四、这类"自我进化"诉求不要实现成"直接改源码"。说明你会把它落在哪条既有链路上，
    以及如果给它开一条旁路，为什么没有任何东西会报红。

五、写断言时逐条自查：
    Q1 这条断言在什么条件下会变红？（答不上来它就不是检查）
    Q2 有没有一个**只有它**会命中的输入？如果没有，说明它可能被邻居顶替。
    Q3 「缺数据必须被说出来」是**每一处**都说了，还是**随便哪一处**说了？
       后者等于允许沉默被别的文案顶替。
    Q4 反向有没有检查？（只做正向检查的话，"既说了没有数据、又顺手报个第一名"
       的实现能全绿）

六、每个新增断言都做一次破坏验证，确认它真的会红，且红的是**它自己**那条。

七、最后真起服务、真发那几句话，看整条链上的实际回话
    （纯函数烟测不够 —— 用户的抱怨发生在整条链上）。
```

### L6.20.8 本层的明确不做

```text
1. 不把演示数字接成"真数据" —— 缺的是**数据源**，不是展示。
   造一份分策略盈亏出来才是真正危险的。
2. 不给管家开"直接改源码"的意图（理由见 L6.20.4）。
3. 不让类别判定兜底猜 —— 心法类别判错会以错误类别回注进每一次提案的上下文，
   那是长期污染面。判不出来就拒绝，并列出合法值。
4. 不做"一句话一管道"的读取 —— 所有读取必须过注册表，
   否则"绕过统一措辞"那条缝会重新长出来。
```

---

## L6.21 因子批量生产线：**能被拒绝，才算一条产线**（2026-09-18 · 第十二轮）

> 来源：§3.23。用户要求"AI 量化因子批量生产达到商用级"，并指定先对标 5 份研究资源。
> 完整对标报告：`docs/FACTOR-PRODUCTION-BENCHMARK.md`。

### L6.21.1 一句话定位

**因子数量不是瓶颈，"能拒绝它们"才是。** 上游一个 skill 就能生成 2,640 个候选，
而一个不设门禁的因子工厂产出的是一份**无法审计的清单** —— 它没法回答"为什么这 4 个留着"。

### L6.21.2 ★ 最值钱的一条：照抄之前先问「这个统计量在这里真的成立吗」

上游（日频、几百到几千只票）的核心统计量是**每日横截面 Rank IC**。照抄到本项目会得到：

```text
标的池 = BTCUSDT + ETHUSDT  →  横截面宽度 = 2
宽度 2 的横截面秩相关只有 {-1, 0, +1}；每日 1 个观测
⇒ 算得出"IC 序列"、算得出 ICIR、数值**看起来完全正常**，只是它测的东西不存在
```

⇒ 换**时间序列 Rank IC**（单标的 · 按 horizon 取前视收益 · 窗口内取均值/标准差）。
★ 这是判据 5 的最难形态：**不是"跑不起来"，而是"跑起来一切正常但测的是空"**。
写这类代码前先算一遍"横截面有多宽"，宽度 < 30 就别用横截面统计量。

### L6.21.3 判据落在服务端，且**调用方传不进阈值**

```text
POST /factors/generate   ← 只接受 count / dryRun；**拒收 body 里的任何阈值**
GET  /factors/index      ← 三态披露（total / accepted / rejected / unverifiable）
阈值只住在 src/engine/factorEval.ts 的 DEFAULT_FACTOR_GATE + 消费方配置
```

理由与凭据范式同源：**只要阈值能从请求里进来，一次 POST 就能把"不过"变成"通过"**，
而账本上看起来只是"某个调用方用了自己的阈值"。

### L6.21.4 三态门：`null` 不是"通过"

```text
① 有未来函数               → rejected
② 未来函数判不出来(null)   → unverifiable    ← ★ 最容易写错的一格
③ 来源 ≠ history           → unverifiable
④ bar 数 < minBars         → unverifiable
⑤ 覆盖率 < minCoverage     → rejected
⑥ 换手 / 重复 / |IC| / ICIR → rejected
⑦ 全过                     → accepted
```

★ **顺序本身是设计**：证据不足（②③④）必须排在业务不及格（⑤⑥）之前 ——
否则"我判不出来"会被记成"它不好"，而这两件事对下游的含义完全相反。

未来函数判定的实现方式 = **截断复算**：切到 t、重算整条序列、逐值比第 t 个（相对误差 > 1e-9 即判有）。
返回类型是 `boolean | null`，**不是 `boolean`** —— 类型上就堵住"没判出来当通过"。

### L6.21.5 去冗余：写进枚举 > 写进过滤器

上游用字符串 slug 去重，漏掉"秩等价"的重复（`z(x)` / `rank(x)` / `tanh(x)` 与 `x` 同序不同名）。
本项目**两层都做，而且实测证明两层各自都会漏**：

```text
① 枚举层：FACTOR_TRANSFORMS **剔掉仿射变换**（z-score / 全样本标准化）
   —— 秩相关在仿射变换下不变，留着它们只是把 33 个机制放大成 2640 个"因子"
② 评价层：Spearman 秩相关判重（maxDuplicateCorr = 0.9）
   —— 因为枚举层挡不住**单调非线性**变换
```

★ 实测抓到枚举层漏的那一半：`compress` 的实现就是 `Math.tanh`，**单调** ⇒ 与 `raw` 秩相关 **1.000**，
`ret_compress_4` 被 `duplicate` 拒掉。
⇒ **"写进枚举"只省算力，不能单独成立；过滤器会漏，枚举也会漏，两层合起来才不漏。**

实测：40 候选里 **15 个**被秩相关拒掉，且没有一个是同名重复（同名早被 slug 拦了），
⇒ 说明这 15 个在名字串口径下**本来会全部混进台账**。

### L6.21.6 ★ 别让"选"和"判"吃两批数据（本轮顺手修掉的既有缺陷）

```text
提案引擎 buildCandles()  →  小时种子的合成 GBM
过拟合门 evidence.ts     →  data/history/*.json 真实行情
⇒ 门禁的数字是对的，但**它判的候选从来不是真实数据上选出来的**
```

修法：`buildCandles()` 改调 `loadEvidence()`（既有函数，不另写取数），
真实历史 ≥120 根即用真实历史并标 `origin: history:...`，否则回落合成并**写明** `dataOrigin`。
★ 与 F-47 同型（范式 6）：**同一个决策链上的两步必须吃同一批数据**，且有断言钉住（F10 比 `dataHash`）。

### L6.21.7 门禁与破坏验证

```text
门禁 29 → 30 道（+`test:factors`，10 场景 F1~F10，全离线）
  关键是"每一格都能变红"：6 个阈值逐个推到极端 → 结论必须按预期翻转（minBars 记 unverifiable，其余记 rejected）；
  全开 → 必须通过（反向对照，否则"门坏了"会被读成"阈值被读到了"）
  端到端那组**自带前置检查**：历史文件在 / 门可达 / dryRun 不写盘

真跑生产台账**刻意不进 CI**：`npm run factors:run` 手动跑（进 CI ⇒ 每次跑 CI 都在改数据）

破坏验证 15/15（`_mutate12.mjs`），第一遍逃逸 2 组 —— 且两处都在**我自己写的夹具上**：
  ① 用"全部门都打开"的常量去验去冗余 ⇒ 被测闸也开到了 1.5，该断言在自己的场景里不可能触发
     （判据 5 的镜像：不是"代码里不可能发生"，是"**在我这个夹具里不可能发生**"）
  ② 两处变异被更早的场景抓住，而我只记了单一预期场景 ⇒ 报表读成"逃逸"
     ⇒ 预期改成数组：**"没抓住"和"被别的场景抓住"是两件事，报文分不清就等于没区分**

第十三轮收尾补（`test:factors` 10 → 14 → **15 场景**，CI 30 → 31 道）：
  ① 汇总口径三态 —— 毛/净取**各折最小值**、成本拖累取**各折均值**（`summarizeStrategyFolds`，唯一实现）
     ★ 夹具必须让"两个最差折落在**不同的折**上"，否则这条检查是**空转的**（判据 5）
     ★ 代价：**同一个事实的两份副本** —— 面板三列并排却不写"均值"，用户照着相减会算出
       一个与引擎不一致的数（实测 54.23 vs 58.20）。⇒ 列名做成**导出常量**而不是写在 JSX 里：
       写在 JSX 里就会被缩写回去，而缩写之后**没有任何门禁会红**
  ② 破坏验证 M13~M16 **4/4**（`_mutate17.mjs`）：min→max / 均值→单折 / 列名缩写 / 删"不可相减"
  ③ **第十四轮补**：`test:factors` 15 → **16 场景**（+F16）。
     ★ F16 是**配对断言**的范例：默认（现货）模式下"一直看空"的策略必须 **0 笔成交且权益恰好不动**
       （证明默认没变），同一夹具 + `allowShort: true` 在下跌 20% 行情里必须拿到 **+18.6%**
       （证明做空真的接上了）。**只留哪一条都不够** —— 只留后者，任何改动都可能让它变绿（判据 18）；
       只留前者，测不出做空到底生效没有。**两条结论相反才是这条检查的力。**
     ★ 做空是**可注入选项**（默认 false = 现货语义），不是改默认值：
       默认值一改，全系统回测口径同时漂移，而漂移是**静默的**。
     ★ 实证方法：`backtest:golden` 红了之后**先写差异定位脚本逐字段对比**
       （实测结果：除 `engineVersion` 外 winner 的 8 个数值与 leaderboard 20 条**逐字节相同**），
       **证明是预期变更之后才允许 `--update`** —— 否则一次"顺手更新基线"
       会把"改坏了引擎"永久掩盖，此后再没有任何检查能发现它。
     ★ 别把「毛收益为负」直接读成「因子方向不成立」：策略是纯多头时，
       下跌行情里毛收益**数学上不可能为正**。实测 BTC 12 个月买入持有 −34.01%、5 折里 3 折下跌 ——
       那个观测量在两种事因下长得一样（判据 14）。见 §3.25.5。
```

### L6.21.8 复现这一层时的提示词（可直接粘贴）

```text
我要给一个"已经有风控与过拟合门"的交易系统加一条因子批量生产线。先别写代码，按顺序做：

一、先说清"这条产线要能拒绝什么"。如果只能生成不能拒绝，它产出的是一份无法审计的清单，
    而不是能力。列出你会设的每一道闸，并为每道闸回答：**它在什么条件下会变红？**

二、在照抄任何外部因子库的统计量之前，先算一遍这个统计量在本项目的样本结构下是否成立。
    本项目的标的池只有 BTC 与 ETH —— 请说明横截面类统计量（横截面 IC / 中性化 / 分层回测）
    在这里会发生什么，以及你会换成什么。

三、判定档位用三态：通过 / 拒绝 / 证据不足。要求：
    1) "证据不足"必须排在业务不及格**之前**（否则"判不出来"会被记成"它不好"）；
    2) 判不出来的返回 `null` 而不是 `false`，**类型上就堵住"没判出来当通过"**；
    3) 每一档都要能说出理由（理由进台账，可核对）。

四、阈值只许住在服务端；接口**拒收请求体里的阈值**。说明为什么（提示：否则一次 POST 就能把"不过"变"通过"）。

五、去冗余：先用"枚举"排除与原始值秩等价的变换，再加一层秩相关兜底。
    说明为什么"写进枚举"比"事后加过滤器"可靠。

六、把这条线的取数接到**既有的**证据加载函数上，不要自己写一份取数。
    并加一条断言：这条线与过拟合门拿到的是**同一份数据**（比内容哈希）。

七、门禁：每一道闸都要能变红（逐个推到极端看结论是否翻转），并加一组"全部打开 ⇒ 必须通过"的反向对照。
    会写生产台账的脚本**不要进 CI**。

八、每个新增断言做一次破坏验证，确认红的是**它自己**那条；预期场景写成数组。
    最后真跑一批，并**如实报出**通过数、|IC| 分布和"这些数字承不承得住"。
```

### L6.21.9 本层的明确不做

```text
1. 不为"通过数好看"调低阈值 —— 那是把门禁改成装饰。
2. 不做遗传/大规模搜索：2,880 根样本上搜索必然过拟合，PBO 门会拦住，
   代价是烧掉大量算力换一堆注定被拒的候选。先解决样本量。
3. 不把 accepted 因子直接接进策略生产（需要另一条选优门，且需用户裁决）。
4. 不把台账做成"更多数字的面板"：先解决数字承不承得住，再谈展示。
```

---

## L6.22 · Agent 舰队：**"不许有孤岛"要变成机制**（2026-09-19 · 第十六轮）

### L6.22.1 一句话

多智能体系统最贵的失败不是"某个 agent 做得差"，而是**"它到底有没有被接上"没人能回答**。
判据 11：**只有测试和探针可达 ≠ 可达。** 一个逻辑完全正确、有断言、有探针、
但**没有任何生产路径会调用**的成员，等于不存在 —— 而且是**静**的不存在。

### L6.22.2 复现顺序（顺序错了会返工）

1. **先把"孤岛"写成判据，再写成员。** 每个成员必须同时声明 `duty` / `reuses` / `emits` / `outputs`；
   每个 `emits` 主题必须**已登记且有订阅者**；每个消费面必须有 `{file, marker}` 证据可核对。
2. **消费面的证据要选"读取函数名"，不要选 `import`。** import 了不用也是绿的；
   而"撤掉接线"这个动作一定会让函数名消失（判据 6：绿必须只有该修复能造成）。
3. **`reuses` 必须解析出真实源码路径** —— 这是"不允许在语音/面板里另写一份实现"的唯一机制（判据 10）。
4. **动作类成员必须声明 `intent` 且进 `DANGEROUS`**，走两段式确认；
   无确认时 `durationMs` 必须是 `0`（一步都不跑，不留半个改过的系统）。
5. ★ **断链 = 整体失败，且判定要按"上游声明的 `emits`"。**
   按"实际发出了什么"判的话，上游静默不发时，这次任务会被降级成"没有上下游"而**报成功** ——
   断链被写成了好事。**这一条是我在写烟测时抓出来的真代码缺陷，不是设计洁癖**（判据 1 的同族）。

### L6.22.3 门禁怎么写（`test:fleet` 22 场景的形状）

- **F1"真表 0 问题"与 F2"坏表逐条变红"必须成对**：9 条结构判据 + 10 条通道/动作判据，
  每条都喂"**只坏一处**"的输入，并断言问题**归属到具体 id**（不是"某条问题里出现了某个词"）。
- ★ **有些分支在真实表上不可达**（例如"消费面指向一个不存在的 agent"）——
  这时要**同时注入坏表与坏注册表**，否则那条判据永远喂不到。
- **F4 快照数字必须来自账本**：只跑一个成员时，只有它从 `never` 变 `ok`，其余仍是 `never`。
  这一条同时防"写死演示数字"和"一跑全绿"。
- **F3 生产入口**：面板断言要匹配**调用形态**（`useFleet(` / `runFleetTask(`），
  不要匹配"DEMO 数据"这类字样 —— 那会命中注释（我第一版就是这么误报的）。

### L6.22.4 失败也要能指出"该动哪一处"（本轮最通用的一条）

候选模型全部失败时，"全都失败了"这句话**至少对应三种事因**，而下一步彼此**相反**：

| 事因 | 该怎么修 | 回话必须说 |
|---|---|---|
| 免费额度打满（429 `free-models-per-day`） | 等额度 / 加额度 | "免费额度用完了" |
| 模型名字腐烂（404 / `Model is unavailable`） | 重跑探针换名单 | "重跑 `npm run llm:probe`" |
| 付费通道没开（`PAID_NOT_ALLOWED`） | 开开关 / 给小时额度 | "付费通道是关着的" |

**复现要点**

1. **别在上游把非 200 抹成一个 `null`。** `chatCompleteParts` 只回 `null` ⇒ 路由把 429 / 403 / 404
   一律记成 `EMPTY_RESPONSE`，三种原因被抹平。要把**状态码 + 原厂报错片段**带上去。
   ★ 顺带一种隐患：`isDeadly()` 里写着 `HTTP 40[234]`，但因为 reason 里从来没有状态码，
   那一支**从来没有被命中过** —— 一条永远为假的判据，与"永远为真的检查"同族、方向相反。
2. **归属判据 = 同一个模型 + 10 秒内。** 换模型时不许借用上一条的失败当理由。
3. **可注入**：`RouterDeps.lastHttpFailure` 做成依赖注入，否则这条判据在测试里**永远喂不到**（判据 1）。
4. **判据只写一份**：三态判定与三份文案都在模块级导出，回话与断言吃同一份。
5. ★ **负向断言的选词**：不要写 `!/名字烂了/` —— 额度那份文案里有一句"这不是模型名字烂了"，
   **它以否定形式包含了那个词**，于是断言对**正确输出**报错（判据 30）。
   要选"别的分支才会出现的**动作词**"。

### L6.22.5 环境自举必须挂在**读取侧**

症状：环境里明明配了 `OPENROUTER_API_KEY`，但脚本 / 探针 / 桌宠独立进程一律报"没有任何可用厂商"。
原因：`ensureEnvProvider()` 只在服务启动序列里调过一次，**不经过那次启动的入口全都拿不到**。
⇒ 把幂等自举闸门挂到 `getActiveLlm()` / `listProviders()` 里。

**判据：凡是"必须经过某个入口才生效"的能力，那个入口就是 bug 的温床。**

### L6.22.6 探针与生产必须走同一条路径

上一版视觉探针自己手写 fetch，答对了"红色"；而生产链路用同一个模型回 **400**。
⇒ 探针必须调**生产函数 + 生产系统提示词**；候选从 `/models` **动态取**
（硬编码名单会静默腐烂），并留 `CATALOG_VERIFIED_AT`（默认 14 天算过期）。

★ **探针会消耗真实额度** —— 为写文档反复重跑取证，会把免费档当天额度打满，
之后生产链路全红，而它**看起来像"能力坏了"**。取证跑一次、落盘、引用落盘文件。

### L6.25 桌宠对话记录：**"聊过的话"要能被找回**（2026-09-20）

> 用户原话：「和桌宠的聊天记录也需要能查阅」。
> 复现这一层时，下面五条是**踩过才知道**的，不是设计洁癖。

#### L6.25.1 落盘点必须在**会话状态机**，不能在业务分支里

回话出口有 **4 个**（3 处在"确认"处理里、1 处在主流程的 `finish` 闭包）。
在业务层落盘 = 同一个动作四条实现路径，将来加第 5 个出口必漏。
⇒ 落盘挂到 `beginTurn` / `commitReply` —— 任何一轮对话都必须经过它们，
这是**唯一必经点**。并用源码扫描断言钉住"业务层一次都不许出现 `record*Turn(`"。

#### L6.25.2 配对键要带 `sessionId`

轮次号是**进程内**自增，重启后从 1 重来，而记录是跨进程累积的文件。
只按轮次号配对，会把"昨天第 3 轮"与"今天第 3 轮"拼成同一轮 ——
**而拼出来的那轮看着完全正常**。⇒ 每行带 `sid`，配对认 `sid + turnId`。

#### L6.25.3 三组必须分开记的东西

| 要分开的 | 为什么 |
| --- | --- |
| 作废的答复（`dropped` + 原因） | "答了但被打断"与"根本没答"指向相反的动作 |
| 一轮的三种态 | 问了没等到答复（`unanswered`）/ 都齐（`answered`）/ 只有答复（`orphan`，出现即异常） |
| 读不到的四种 | 目录不存在（**真的没聊过**）/ 目录读不了 / 有行坏了 / 写不进去 |

★ `null` 不许退化成 0：`unreadable: null` 表示"真的读到了（哪怕 0 条）"，
有值才是"读不到"。两者在界面上长得一样，下一步动作却相反。

#### L6.25.4 ★★ 烟测的第一件事是隔离，最后一条断言要**检查结果**

初版把隔离放在最后一组，而前面的端到端用例会真的走生产入口 ⇒
**249 行假对话写进了用户的真实记录**，且与真话在界面上没有区别。

⇒ 两件都要做：① 隔离放在 `main()` 的**第一件事**（早于任何会落盘的用例）；
② 加一条**反回归断言**：跑完整个烟测，真实数据目录里**不许出现本进程的 `sid`**。
★ 它检查的是**结果**而不是意图 —— 将来有人插一组新的落盘用例，它立刻红。

#### L6.25.5 ★★ 比较断言先问"两个数来自同一扇窗吗"

端点的列表参数常带 `limit`。**被截断的列表长度不能用来做增减断言**：
`===` 恒真（假绿）、`+1` 恒假（对正确输入报错）。
⇒ 改成**按 id 计数** —— 新产生的那条一定落在"最近 N 条"窗口内，
既不依赖累计长度，也不怕截断。

#### L6.25.6 复现这一层时的提示词（可直接粘贴）

> 给语音/对话加持久化记录：① append-only JSONL，按天分文件，模块内**只允许 `appendFileSync`**
> （无 writeFileSync / rename / unlink），不提供删除接口；② 落盘点放在会话状态机这个
> **唯一必经点**，不在业务分支；③ 每行带 `sessionId`，配对认 `sid + turnId`；
> ④ 作废的答复也要记，且带 `dropped` 与原因；⑤ 读取区分"真的没有"与"读不到"
> （`unreadable: null` vs 有值）、坏行计数、写失败留痕；⑥ 单条正文超限截断但记原长；
> ⑦ 记录根可注入（测试隔离用），烟测的隔离是**第一件事**、并有一条"没污染真实数据"的
> 结果断言；⑧ 只读端点若带 token，前端调用也要带 —— 否则表现为"界面上永远说读不到"。

# Part I · 验证层与 CI（38 步）

> v1 完全缺失。**这一层是"能不能相信这套系统"的答案**——没有它，前面所有架构描述都只是声明。

## L6.23 · 把「能听懂」做成**唯一口径**：跨层归属 + 假绿（2026-09-19 · 第十七轮）

### L6.23.1 一句话

「听懂用户原话」这件事，真正的难点不是词表不够长，而是**同一句话被两层同时认领**。
本轮 CI 就是这么红的：`test:voice` S14 报「系统现在什么情况」期望 `ask_system`、实际 `dispatch_task`。
根因不在那条断言里 —— 而在**两条口径**（判据 8）。

### L6.23.2 复现顺序（顺序错了会返工）

1. 先让**任务类**原话走唯一口径（`planTask()`），语音层不再自己维护白名单。
2. 再处理**只读类**原话的归属 —— 这一步一定会暴露重叠，**不要一条条试**，
   直接做「两个表全量比对」（§L6.23.4）。
3. 最后才加能力（迭代挖掘 / 自治循环 / 清理 / 自学习）。
   顺序反过来的代价：新能力会落在**归属还没划清**的地基上，等你发现抢占时，
   已经分不清是"新能力错"还是"归属错"。

### L6.23.3 ★ 跨层口径：同一句话只能有**一个主人**

两层认领同一句时的表现是**静默换归属** —— 用户问"系统现在什么情况"，
收到的是四个成员跑一遍的巡检汇报。修复时有三件事必须同时做：

| 动作 | 为什么 |
|---|---|
| 划清归属并**写在两边**（各写一段"谁归谁"） | 只写一边，下一个改代码的人看不到另一半 |
| 把改动带来的**洞堵上** | 计划表不再认「系统」⇒「系统怎么样」会掉到行情兜底，必须由语音层接住 |
| 把归属做成**断言**（不是注释） | 注释不会变红（判据 10：红线要变成机制） |

★ **处置方向必须是"改新越界的代码"，不是"改旧断言来迁就现状"。**
S14 是上一轮就已存在且绿着的契约 —— 把它改成迁就 `dispatch_task`，就是在搬球门。
判断依据很简单：**谁是新来的，谁让路。**

### L6.23.4 ★★ 修好一条 ≠ 修好一类：从**两个表**出发做全量比对

一条条猜"还有没有别的抢占"是猜不完的。正确做法：把**两张真实存在的表**拉出来交叉比对 ——
语音层的意图表（`scripts/voice-smoke.ts`，**不另抄一份**）× 舰队计划表（`FLEET_TASK_PLANS`）。
判据：**凡是"期望意图不是 `dispatch_task`"的行，`planTask()` 都不许认领。**

这一步当场又抓出两条同类（都出自 `propose` 计划里那个**裸 `/进化/`**）：

| 被抓走的句子 | 原本 | 被认成 | 为什么错 |
|---|---|---|---|
| 进化实验室能干什么 | `ask_lab` | `propose`（跑一轮提案） | 用户**在问**能力，被接成**会写东西**的动作 |
| 进化一下你自己 | `self_upgrade` | `propose` | 同一个动作两条路径（判据 8） |

★ 顺带补一条通用判据：**「能干什么 / 能做什么 / 干什么用 / 有什么用」是最典型的"问能力"句式，
却不含 `吗/呢/哪些/怎么样` 里任何一个字** —— 只按常见疑问词写 `looksInterrogative()` 会漏掉它们，
于是 `writes: true` 的计划会把一句提问接成一次写操作。

### L6.23.5 ★ 假绿：**能变红 ≠ 红的是你要证的那件事**

`F15 零过门会真的换组` 原来写 `setsTried >= 2`。把"连续零过门就换组"整段改坏之后，
**它仍然是绿的** —— 因为"试到第二组"靠**空间挖干净**那条路也能满足。

⇒ 换判据要选**只有这一条路能救活的输入**：夹具里基线组共 12 格、每轮取 4，
所以"不靠零过门判定"要**用满 4 轮**才让位，而靠它只要 **2 轮** ⇒ 断言写「基线组被用了几轮 = `BARREN_LIMIT`」。
为此把 `BARREN_LIMIT` 导出而不是在测试里再写一个魔数。

**同类还有第二条（夹具选错 = 对正确的输入报错，判据 2）**：修好归属之后，
我顺手把只读用例选成了「系统现在什么情况」—— 而这句话的主人本来就是语音层。
⇒ 选夹具和选判据是同一件事：**先问"这句话归谁"，再问"它该落哪"。**

### L6.23.6 迭代挖掘：四个退出态，其中一个专门治**假迭代**

`target-reached` / `all-sets-exhausted` / `max-rounds` / **`index-not-advancing`**。

最后一态来自实测：`dryRun` 下台账不推进 ⇒ 第二轮"扩出来的候选"与第一轮**逐字相同**，
输出看起来像"换了几组结果都差不多"，而真相是**第 2、3 轮什么都没发生**。
它同时兜住真实的写盘失败（磁盘满 / 文件被锁）—— 那两种事因在这里同构。

★ 还有一处更朴素的教训：换组那一段**原来是只打日志、没有真的换组**的
（注释与日志描述了一个没有实现的逻辑）。实跑 6 轮全耗在同一个窗口组上、只试了 1 组 ——
而"必要时换因子族"正是需求原话里的一句。**日志说做了 ≠ 真的做了。**

### L6.23.7 可逆自治循环：不可逆动作**不许**进循环

自治循环里只允许可逆动作（移动文件 / 产提案单），不可逆的（删文件 / 下单 / 改代码）
必须人工确认。落地成机制的两条：

| 机制 | 形状 |
|---|---|
| 任务必须声明 `reversible` | `false` 的任务进循环直接拒（`IRREVERSIBLE_JOB`），**不是靠纪律** |
| 清理只 `renameSync` 到 `.trash/<时间戳>/` | 源码扫描钉住"零 `unlink`/`rmSync`" —— 可逆性是这个模块进循环的**唯一理由** |
| 启动幂等 + 停机清 `nextAt` | 重复启动返回"已在运行"；停机后面板不许显示"下次触发：过去某时刻" |
| 时钟与执行器**可注入** | 否则"到期真跑"这件事只能靠等真实时间，测试写不出来 |

### L6.23.8 复现这一层时的提示词（可直接粘贴）

```
给这个系统加「一句话 → 任务链」的能力时，请遵守：
1. 「能不能接、接了干什么」只能有一份实现（一个 planTask()），语音层与 HTTP 层调同一个函数。
2. 只读类意图的归属必须写成两边各一段说明 + 一条断言；同一句话不许有两个主人。
3. 加完任何"能听懂"的新说法，必须做一次跨表全量比对：把语音意图表逐条喂给 planTask()，
   非 dispatch_task 的行一句都不许被认领。（照抄这张表，不要另抄一份句子清单。）
4. 每条新断言都要回答「改坏哪一处会让它变红」，并把这个变异写进变异脚本跑一遍。
5. 会写东西的计划遇到疑问句必须不接；注意「能干什么」这类句式不含常见疑问词。
6. 进自治循环的动作必须可逆；不可逆的必须人工确认，且用代码拒绝而不是用文档提醒。
```

### L6.23.9 明确不做

① 不把有副作用 / 联网 / 耗额度的一次性脚本放进 CI（外部依赖 ⇒ 随机红）；
② 不为了"让 CI 变绿"改旧断言 —— 见 §L6.23.3 的"谁是新来的谁让路"；
③ 不在归属划清之前堆新能力。

## L6.24 · 长开的自治循环 + 免费账号池 + 「运行态必须现读」（2026-09-19 · 第十八轮）

### L6.24.1 一句话

这一轮的四件事有一个共同形状：**系统说自己是什么样，必须是从活着的系统里读回来的**。
"自循环已常开""账号池已就绪""新闻雷达会定时读" —— 只要其中任何一句是**常量**，
它就会在额度爆掉 / 循环被关掉 / 账号没配的时候**照样这么说**。

### L6.24.2 免费账号池：按账号记账，而不是按模型名

| 规矩 | 落法 | 反例的代价 |
|---|---|---|
| 一个 key 一个账号 | 判重键 `(baseUrl, apiKey)` | 用 `baseUrl` 判重 ⇒ 同家的第二个账号**永远注册不进来** |
| 额度账按**账号 + 当天** | 落盘 + `until` = **本地**次日零点 | 写 UTC ⇒ "明天恢复"落在本地下午 8 点 |
| 与"模型名拉黑"**分开记** | 换账号按天、换模型名 10 分钟 | 合成一份 ⇒ 一次 429 把整个模型名拉黑，或反过来一直重试同一个死账号 |
| 爆掉的账号**排序靠后，不删** | 失败链里留 `ACCOUNT_EXHAUSTED` | 删掉 ⇒ 理由退化成 `NO_PROVIDER` ⇒ 把用户引向"去配厂商"（方向完全相反） |
| 池子里**每个**账号都要能列出来 | `snapshot.accounts`（不只 `exhausted`） | 只列爆掉的 ⇒ 池子安静时"我新加的 key 认到没有"**没有出口可问** |

★ 加账号**不改代码**：`<KEY>_2` … `<KEY>_9` 同族序号，或一条显式条目（`名字|baseUrl|apiKey[|模型]`，分号分隔）。
解析失败的条目**不静默**（拼进 note），否则用户会以为"我明明配了"。

### L6.24.3 自治常开：默认开，但**该跳过的时候要说出来**

- 默认常开，只有显式 `EV_AUTONOMY=off` 才关；启动**幂等**（重复调用不重复排程）。
- 挂在启动序列里、且必须在"成员都装好"**之后**（循环里每一项都经由执行器跑）。
- ★ 需要模型的排程遇到"额度全爆"要**跳过并单独记一列**（`skippedCount`），
  **不许记成失败**：跳过 = 等额度回来；失败 = 这一项坏了要去查 —— 两件事指向相反动作。
  混在一起的话，额度爆掉那天会显示"自学习连失败 3 次"，而系统一件事都没做错。

### L6.24.4 ★★ 启动器不许念常量：三行运行态全部现读

双击启动器最容易犯的错是**把想说的话写成固定文案**。正确做法：

1. 抽一个**纯函数**渲染（`renderRuntimeSummary(probe)`）—— 纯函数才可以用"构造出来的输入"
   断言"读不到时会说什么"；
2. 数据由启动器去问刚起来的服务（`/fleet`、`/fleet/autonomy`、`/fleet/news`）；
3. ★ `null` **不许退化成 0**：读不到就写"读不到（原因）"，并显式说明"这不等于没有"——
   "零个账号"与"没读到"指向相反的动作，显示成同一个样子就等于把用户往错方向推。

| 断言（`test:stack` S-A1~S-A5） | 它守的是 |
|---|---|
| 读不到时三行都要标注，且**不许出现"运行中""N 个账号"** | 不许把"没读上"伪装成"一切正常" |
| "最近一项"按**时刻**挑，不按数组顺序 | 夹具刻意把"更晚的那项"放在数组前面 ⇒ 取 `[0]` 的实现在两条断言上同时红 |
| 新闻行认的是 `news_watch` 那一项 | 别的任务的时刻**一次都不许出现** |
| "最新的一条"取**末尾**（`latestDigest()` 是时间正序） | 取 `[0]` 会印出最旧的那条，而它看起来完全正常 |
| 没标题不印空引号；0 条要**说出来** | 沉默会被别的文案顶替 |

★ 顺带修掉一处真缺陷：**令牌有两个来源** —— 启动器读 `process.env.ORCH_TOKEN`，
而子进程拿的是 `stackRoles()` 的 `DEV_TOKEN`。两个表达式在绝大多数机器上得到同一个字符串，
所以看不出问题；但外部 shell 一旦导出 `ORCH_TOKEN`，启动器就会拿**服务不认的令牌**去问，
得到 401，然后把它解释成"读不到运行态"。⇒ **同一件事的凭据只能有一个来源**。

### L6.24.5 新闻雷达：判相关用确定性规则，模型只写提案

分工是硬的：**打分是规则，措辞才是模型**。理由是"判相关"必须可复现、可核对、可解释
（每条带 `matched` 与 `reasons`，要能回答"为什么觉得这条相关"）。

复现顺序（顺序错了会返工）：

1. 先把**抓 → 打分 → 去重 → 落盘**做成纯确定性的一段，能离线断言；
2. 再让模型**只**做最后一步（写内化提案），并且**提示词的输出格式必须与解析器同构** ——
   实测坑：提示词写"输出 JSON 数组"、解析器只认三行文本 ⇒ 模型回 `[]` 被**判成"没读懂"**。
   修法不是加解析分支，而是让两边对齐（判据 8），并把 `[]` / `{}` 认作"无需改进"这一**第三态**；
3. 最后才挂排程 —— 并把它的 goal 写进计划表，**放在会抢词的计划之前**
   （裸 `/提案/` 曾把这个排程的 goal 抢走）。

★ **先落盘再内化**：抓到的条目当天一份 JSONL，同 id 不重复内化；额度过载时"内化"这一步跳过，
但**摘要仍然落盘**，并在回话里说清跳过的原因（不是静默少做事）。

### L6.24.6 明确不做

① 不把有副作用 / 联网 / 耗额度的一次性脚本放进 CI；
② 不改上一轮已绿的断言来迁就新代码；
③ **没让启动器去"保证"任何它读不到的东西** —— 说不出的事就说"读不到"。
## I1 · 门禁清单（`npm run ci`，实测 38 步 = 31×`test:*` + 7 道其它）

> ★ 2026-09-16 建立 · **2026-09-17 第九轮 29 道逐道实跑复核**（不是"跑一次没报错"——那是 `&&` 短路链，见 §I3 与 §3.6）。
> ★ **2026-09-18 第十三轮 31 道逐道实跑复核**（同一方式；本轮新增 `docs:tables`，`test:factors` 同轮由 10 → 14 场景）。
> ★ **2026-09-18 第十四轮 31 道逐道实跑复核**（同一方式；`test:factors` 由 15 → **16 场景**，新增 F16 对称多空）。
> ★ **2026-09-19 第十六轮 32 道逐道实跑复核**（同一方式；CI 新增 `test:fleet` **22 场景**，`test:voice` 15 → **16 场景**，`test:model-router` 12 → **15 项**，`test:factors` 16 → **17 场景**）。
> ★ **2026-09-20 第十九轮 38 步逐道实跑复核**（同一方式；CI 新增 `test:ui` **39 断言** —— 界面动作通道。★ 本轮顺带**修掉这张表自身的一处漏登**：把表里 34 行与 `npm run ci` 的实际链逐条比对，差的正是 `lint:negation` / `test:contract` / `test:provider-health` 三道（**前几轮就漏登了**）+ 本轮新增的 `test:ui` ⇒ 已补齐并重编号。这类"同一事实抄多份"的漂移，与 F-21 同族，但表本身是**清单**、漏登的后果比写错更难发现：漏掉的那道门**不在任何人的视野里**）。
> ★ **2026-09-20 第十九~二十轮 35 道复核**：链上新增 `test:contract`（19 场景）与 `test:ui`（**59 断言**，含 U24~U27），`test:voice` 17 场景（+S15：严格档/宽档顺序 · 危险意图否定护栏 · 唯一实现）。★ 另加两条纪律进 I3（第 6、7 条）：**多窗口下"谁按的"要可比对**、**断言要先能变红**。
> ★ **2026-09-20 第二十一轮 38 步逐道实跑复核**（同一方式；`test:voice` 17 → **18 场景**，新增 **T 组 · 22 条断言** —— 桌宠对话记录。★ 本轮修掉两条**潜伏十几轮的 e2e 假红/假绿**：`/ui/actions` 的 `tasks` 被 `limit: 40` 截断 ⇒ 拿 `length` 比较的断言一处恒假（对正确输入报错）、两处恒真（假绿），已全部改成**按 `actionId` 计数**）。
> 断言数均为实跑输出取值，非文档抄写。本表随语音层落地由 24 → 25，随桌宠层落地 25 → **26**，随合并/keyScope 轮 26 → **27**，随任务层落地 27 → **28**，随进程监督取证轮 **28 → 29**，随因子生产线轮 **29 → 30**，随策略层归因 + 表格校验器进 CI 轮 **30 → 31**（+`docs:tables`），随 Agent 舰队落地轮 **31 → 32**（+`test:fleet`）。
>
...
| 3 | `docs:tables` | **10 文件 / 208 张表** + 反向对照 | 文档表格结构（`\|` 转义 / 列数一致 / 无裸换行）。★ **自带反向对照**：坏夹具必须报出 3 处、干净夹具必须报 0 处 —— 一条不会红的检查等于没有 |
| 4 | `test:orch` | — | 风控 / 熔断 / 晋升闸 / 资金帽 / 握手门 / **killswitch 三处同步** |
| 5 | `test:gateway` | **9 场景** | killswitch 与故障演练（归档 `artifacts/drill-latest.json`） |
| 6 | `test:promotion` | — | 引擎晋升状态机全链路回归（含**拒绝旧自报格式**的回归） |
| 7 | `test:recon` | **3 场景** | 对账逻辑与出站闸触发 |
| 8 | `test:mirror` | **3 场景** | 镜像互查（内存链 vs 持久链）+ **`/events` 读取语义（S5：默认取最新 / 传 since 才是翻页 / 截断自报 / limit 有上限；成对断言，避免"取错了一段"还全绿）** |
| 9 | `test:proposal-slo` | **5 场景** | 提案安全边界 + SLO 评估 |
| 10 | `test:surveillance` | **6 场景** | 操纵自监控标记 |
| 11 | `test:voice` | **18 场景** | 语音层：意图解析 / 中文数字归一 / **语音单与直连拿到逐字相同的拒绝理由** / 两段式确认 / 打断作废 / 播报限流 / **P0 不可压制** / 异动 / **播报溯源** / 配置与音色 / 端到端 / **文案卫生（不得出现占位符垃圾）** / **出声链路（拟人音色 · 报警不许重试失败过的云端 · 口型时间轴）** / **S1b 介绍自己与答非所问（"你是谁"5 种问法全接住 · "介绍某物"不许误接 · 用户原话判成任务而非报价 · 槽位不全也接住 · 真问价仍是问价）** / **S14 系统实况与工具注册表（三态双向可证 · 排行口径显式且 rejected 垫底 · 阶段中文名零缺口 · 「盈利最高」正解为 unavailable · 三份文案各自欠的披露逐份对账 · 自我介绍交代改代码边界 · 未确认不执行 / 确认后过宪法红线且不污染心法库 · 6 个坏工具逐条归属报红）**/ T 对话记录（生产入口落盘 · 作废答复带原因 · 跨会话同号不串轮 · 三态互不顶替 · 坏行/读不到/写失败各说各的 · 只允许 appendFileSync）（见 L6.11 / L6.15 / L6.19 / L6.20 / L6.25） |
| 12 | `test:pet` | **91 项 / 13 组** | 桌宠桌面外壳：窗口标志位 / 置顶层级 / **可找回性不变量** / 越界钳制 / 原生拖动 / 回声闸门 / 口型 / 照片裁剪 / 透明窗三处同向 / 主进程结构不变量（含 **冒烟判定靠协议行 + 存活轮询**）/ **P0 报警对比度** / **P12 单一入口·单一通道·单一实现** / **P13 头像尺寸有界·素材分流·动图不叠器官层·档位严格递增·最大档不超窗口 60%**（见 L6.12 / L6.13 / L6.19） |
| 13 | `test:keyscope` | **21 项** | **密钥权限范围自检**：Binance 三态（含"缺字段≠安全"）/ OKX `perm` 四形态（按逗号切分，不做子串匹配）/ **缺证据一律不放行** / **调用方无从自报结论** / 聚合器**最坏优先**（见 L6.14） |
| 14 | `test:sandbox` | 3 场景 | 沙箱：良性 / **四条出网通道全 BLOCKED** / 死循环击杀 |
| 15 | `test:autopilot` | **9 场景** | 自治：目标越界 / 防重入 / 选优 / 执行 / 止盈 / 回撤停机 |
| 16 | `test:llm-provider` | **5 场景** | LLM 适配器契约 |
| 17 | `test:commercial` | — | 商用加固：哈希链 + **篡改定位** / 指标聚合 / 提案边界 |
| 18 | `test:r20` | **40 项** | R20 内化 5 模块（见 Part L） |
| 19 | `test:risk-guard` | **29 项** | 风控内核：1R 定规模 / 止损几何 / 保本棘轮 / 预留与拦截管线 / 宪法红线 / **心法样本量不可自报（F-44）** |
| 20 | `test:trusted-seam` | **49 项** | 接缝 6 模块：成本闸门 / 对手方三档 / 跨通道结算 / 审批在环 / 声称核验 / 上下文预算 |
| 21 | `test:dex` | **11 项** | DEX 五链：合约完整性 / 规模匹配池深 / **拒绝侧正确** / 同源校验 |
| 22 | `test:leverage` | **29 项** | 微资金 × 杠杆：`maxSafeLeverage` 反推 / 强平几何 / 张数换算 / 通道地板拆分 |
| 23 | `test:verifier` | **18 项** | 独立复核：上下文隔离 / 独立重推 / 任一不符即拒 |
| 24 | `test:model-router` | **20 项** | 模型分层：未识别按付费 / `EV_LLM_ALLOW_PAID` 默认关 / probe 不走付费 / **账号池（甲账号额度爆 ⇒ 换乙账号；次日零点复位；只有额度类才停用；"没额度"与"名字烂了"两种事因分开；环境变量多账号）** |
| 25 | `test:overfit` | **30 项** | **过拟合门禁**：CSCV-PBO 正确性 / 凭据不可伪装 / 真实引擎接线（见 §3.10） |
| 26 | `test:mission` | **17 组（S-M1~S-M17）** | **任务层**：目标解析 / **缺槽位不补默认值且不误报** / **三态均可达且"证据不足"记 `hold`** / 场所名即契约 / **不可行必带数** / 达标数学（119 笔） / **杠杆不放大仓位** / **内容寻址的边界（同义换说法不换 id、少一个从句必换 id）** / 一任务一执行线 / 仅追加前缀校验 / **脱敏零误报（内容地址不被当密钥）** / 门结论倒扫账本（pin 绕过不是通过） / 端到端（见 L6.16） / **S-M14 启动口令：8 条拒绝各配一条放行、TTL 边界、连错作废、新码覆盖、单次消费、明文与哈希都不落账** / **S-M15 端到端启动（不可行不签发 → 可行才签发 → 错码被拒且不动状态 → 正码真的启动 → 不能重放 → 不与下单确认串台）** / **S-M16 文案卫生（检查器自身会报红 / 四类输入无 markdown 星号·叠标点·undefined·NaN / 回落时不自作主张）** / **S-M17 裸倍数与执行诉求（「翻倍」=2 倍 · 6 种裸倍说法 · 「10倍杠杆」不误当收益目标 · 1 倍不算目标 · 3 条执行诉求接住 + 4 条闲聊/问价不被拖进裁定 · 语音层同一句话判成任务而非报价）** |
| 27 | `test:stack` | **11 组（S-K1~S-K6 + S-A1~S-A5）** | **进程监督取证**：**S-K1 三种事因的退出状态逐字节相同（自己 exit / taskkill 打死 / 未捕获异常 → 都是 `code=1 signal=null`；配 `exit 7` 对照证明码是真读到的）** / **S-K2 自崩溃留下遗言（`crashed=true` + 记录文件里真的有原因，不只管道里）** / **S-K3 被打死 ≠ 崩溃（armed 过、无遗言 ⇒ `not_self_crash`，并断言它的退出状态与 S-K2 相等）** / **S-K4 没 armed ⇒ `unknown`，且渲染文字里不许出现"不是自崩溃"（不许下假结论）** / **S-K5 成对：同一夹具自己退出必须告警、被 shutdown 必须不告警** / **S-K6 时间戳是本地时间且带偏移** · **S-A1~S-A5 启动器运行态现读（读不到必须说读不到、不许冒充"运行中/有账号" · "最近一项"按时刻挑不按数组顺序 · 爆掉的账号不许说成可用 · 有确切恢复时刻不许说"明天" · 新闻行认 `news_watch` 且"最新"取末尾 · 没标题不印空引号）**（见 L6.18 / L6.24） |
| 28 | `test:factors` | **17 场景（F1~F17）** | **因子批量生产线**（第十二轮）：未来函数三态（`null` **不是"通过"**）/ 常量与空信号 → `rejected`（有数据但没用 ≠ 没证据）/ 来源三态（非真实数据永不出 `accepted`）/ 判定顺序（证据不足排在业务不及格之前）/ **阈值敏感性（6 个阈值逐个推到极端：minBars 记"证据不足"、其余记拒绝；全开必须通过）** / 秩相关去冗余 / 批量确定性与台账 upsert / 端到端真历史（自带前置检查）/ 台账损坏 → 三态披露 / **与过拟合门同源（同一个 `dataHash`）** / **F11 基因空间与 compute 一致（每个机制在它的最小合法窗口上都产得出值、展开 485 个候选无一越过下限）** / **F12 陈旧判决被标出（覆盖率不足记 `unverifiable`）且干净台账零误报** / **F13 策略门归因（毛/净/成本拖累三个量各自可观测：归因三分支各走一次 + 真跑验「毛−净=成本拖累」恒等式 + 零成本对照）** / **F14 策略层消费的时效性（指纹不符拒进池、定义缺失拒，且每条说清原因）** / **F15 汇总口径三态（毛/净取各折最小值、成本拖累取各折均值；夹具专门让两个最小值落在不同的折上 ⇒ 证明「相减 ≠ 均值」· 空折数组三个量必须都是有限数且胜率为 `null` · 列名常量必须带"均值"、读法说明必须含"不可相减"）** / **F16 对称多空（配对断言：默认现货模式下"一直看空"必须 0 笔成交且权益恰好不动 · 同夹具 + `allowShort: true` 在跌 20% 行情里得 +18.6% ⇒ 两条结论相反才证明做空真接上了 · 涨 4 倍判爆仓 · `defaultStrategyConfig()` 键集必须覆盖 wf 全部字段）**（见 L6.21 / §3.24 / §3.25） |
| 29 | `test:fleet` | **29 场景（F1~F17）** | **Agent 舰队**（第十六轮）：真注册表 0 问题 · **结构类 9 条 + 通道与动作类 10 条逐条喂"只坏一处"的坏表并变红**（问题必须归属到具体 id）· 生产入口齐全（3 个 ui/voice 消费面 + 4 条 HTTP 路由 + 面板换成真实取数）· **单成员真跑落账本且快照数字来自账本（只跑一个成员时其余必须仍是 `never`）** · 上下游真交接 + **断链即失败（按上游声明的 `emits` 判）** · `act` 类无确认必拒且 `durationMs === 0` · 听懂/听不懂 · 体检只读且分组夹具验证 · 成本档 · 语音消费面 · **F13 自治循环（4 项按周期排上、到期真跑并留账本、重复启动幂等、不可逆任务被拦）** · **F14 可逆清理边界（保留名单 / 逃逸 / 子目录 / 未到期 / 未知扩展名五类都被挡下；试跑不动文件；真跑可原样找回；上限生效）** · **F15 迭代挖掘（达标 / 挖尽 / 轮次用尽三态分开；连续 2 轮零过门真换组；只筛本轮批次；台账没推进就停；负号保留）** · **F15b 直连真实筛选器（证明 `slugs` 过滤在真正的筛选器里成立，而不只是桩在遵守）** · **F16 红线机制化（学习模块零写文件调用、清理模块只 rename 不 unlink、提案解析三态可分）** · **F17 跨层口径（语音意图表逐条比对，计划表一句都不许抢）**（见 L6.22 / L6.23 / §3.27 / §3.28） |
| 30 | `test:news` | **17 组（N1~N8）** | **新闻内化雷达**（第十八轮）：相关性判据（门线两侧各一例，且"有词但分不够"不许通过）/ 抓取打分去重 / **内化三态（写了 / 无需改进 / 没读懂）** / 额度爆时跳过并说明 / 追责面（提案能指回条目）/ **不碰源码**（只追加）/ 计划表对齐（`news` 必须排在会抢词的 `propose` 之前）/ 幂等坏输入（见 L6.24） |
| 31 | `test:parity` | **14 组（P1~P3）** | **日报四项内化的落点**（第十八轮）：**P1 同码**（回测与实盘用同一段撮合，server 层不许自己算成交价、摩擦常数非零）/ **P2 日志即事实源**（每次模型调用落盘；`reconstruct()` 三态 `ok / truncated / mismatch` —— **截断与篡改必须分开**）/ **P3 出网拦截告警**（`EGRESS_BLOCKED`（被白名单拦）与 `EGRESS_FAILED`（没通）分开记，不许混）。★ 扫描器必须**先剥注释**：第一版把注释里那句"没有任何 writeFileSync"当成代码命中 |
| 32 | `lint:negation` | 自检 + 全量扫描 | **负向断言的词不许在生产代码里以否定形式出现**：把每条 `=== null` / `=== 0` 断言里选的词拿去全仓扫，凡是查见 `不 X` / `没 X` / `未 X` 的一律报红。★ 它治的是一类**假绿**：负向断言选了一个"在本分支以否定形式出现"的词，于是无论功能好坏它都恒真 |
| 33 | `test:contract` | **19 项（A~J）** | **合约下单槽位**（第十九轮）：`10 美金 125 倍合约做多，止盈 10 成、止损 1 成` 四槽位齐全 / 大写 `U` 识别 / **遮罩（杠杆与止盈止损的数字不许被当成金额）** / 成数 vs 百分之 vs `%` 三种写法 / 现货语义不被污染 / **未解析出的保护要点出来** / **拒绝的三态互不顶替**（没听到倍数 ≠ 倍数越界 ≠ 缺止损）/ 不许静默降级到 1 倍 / 确认值按语义给（保证金 vs 名义额） |
| 34 | `test:ui` | **39 断言（U01~U23）** | **界面动作通道**（第十九轮）：登记表 ↔ 源码**双向对账**（登记了必须真有 `data-ui`；源码里的 `data-ui` 必须都登记）/ `screenText` 必须真实存在于源码 / `speaks` 不含 markdown / **一句话只能有一个主人**（带 `voicePlan` 的按钮不许被按钮通道抢，否则同一句话执行两次）/ 短名字 → `weak` 列候选（而不是回"没听懂"）/ **疑问句不认领** / `writes` 缺确认拒 · **两段式按钮拒且队列不变** · 未知 id 拒并给近邻 / 排→认领→回报 / 第二窗口拿不到 / **TTL 三态（"过期待"不许说成"没排过"）** |
| 35 | `test:provider-health` | 六态分类 + 三动作 | **厂商体检**：`network / no-funds / quota / policy / model-gone / unknown` 六态**分类顺序即优先级**（`network` 最先 —— 否则本机网络故障会被读成厂商故障）/ 三动作 `switch-model / disable / keep`。★ 一条拒绝理由必须能指出**下一步动作**，否则用户只会反复重试同一件事 |
| 36 | `backtest:golden` | 真门禁 | 复现性 / dataHash / **过拟合凭据结构与逐折自洽** / 基线比对 |
| 37 | `build` | — | `tsc && vite build` 产物构建。★ **它守的是一条别的门都守不住的不变量**：import 解析（见 L6.13.4） |
| 38 | `audit:sec` | — | 依赖漏洞（`npm audit --audit-level=high`，实测 0 vulnerabilities） |
...
**后果**：本地 **25 套 smoke 测试**（orch/网关/晋升/对账/镜像/提案/监控/**语音**/**桌宠**/**密钥权限**/沙箱/自治/LLM/商用/R20/风控内核/接缝/DEX/杠杆/独立复核/模型分层/过拟合/**任务层**/**进程监督**/**因子生产线**）
...
6. ★★ 验证"界面真的动了"时：**只有观察到执行的那一窗才算证据**。应用会同时开多个窗口，
   而队列是"取活即认领"（哪一窗先轮询到就哪一窗按），所以"队列说已按下、可我这一窗没动"
   有两种事因、下一步相反：① 通道坏了（要改代码）② 活被另一窗领走且**确实按了**（什么都不用做）。
   分辨法：比对该条记录的 `requestedBy` 与本窗的 `document.documentElement.dataset.uiClient`；
   抢不到执行权就**记"未验证"并说明原因（退出码 3）**，不要记红 —— 会随机变红的门禁会训练人忽略它。
7. ★ 新增/修改断言后做一次**变异**：把被测分支禁掉，确认对应断言变红，且**别的断言不变红**
   （后者才证明它有独立牙齿，而不是被邻居顶替）。
```
...
```text
npm run ci 全绿才放行（30 道，见 Part I）。
...
原文档的「九道发版门」在 Part I 已扩展为 **30 道**（`npm run ci` 全绿才放行）。
**以 Part I 为准**，此处不再复述——避免同一事实两处维护导致漂移。

⚠️ **门禁数量 ≠ 门禁有效**：一个从没被跑到的门（因为短路）与一个红灯的门，
在"给人安全感"这件事上是等价的——**都是零**。
当天实况以 Part I 的逐道实跑表为准（2026-09-16 建立 · **2026-09-18 复跑 30/30 PASS**）。
...
15. L6.9 沙箱                              → 用户代码安全评估
16. 补齐 Part I 的门禁清单（现 **39 道**）   → 每加一层就加对应 smoke，不要最后补
```
...
> F-18 的 lint 已在 v3 清零，但 **GitHub CI 与本地 CI 口径仍不一致**（32 道 vs 5 道），
...
## L6.11 · 交易闸门：同一个动作只许有一条规矩（2026-09-20）

```text
实现 server/tradeGate.ts ——「这笔交易凭什么可以出去」的唯一事实源。

★ 要解决的问题（判据 8）：同一个业务动作不许有两条实现路径。
  实测形态：`runPipeline`（9 道拦截器 + 赔率几何 + 成本 + 组合敞口 + 审批）
  **只有自治循环一个生产调用方**；而人在交易大厅手点的单**一道都不过**，
  直接进纸面撮合 —— 而且人那条路从来没有任何测试覆盖。

★ 修法的方向（这一步比代码本身重要）：
  不要给大厅「再写一份检查」。那是第二份实现，两份迟早对不上。
  ⇒ 把 pkg / context 的构造从 autopilot 抽成 buildMarketPackage() /
    buildInterceptorContext()，让大厅走**同一条** runPipeline。
  ⇒ 门禁要钉住"调用方恰好两个"（源码扫描），多一个就红。

导出：
1. buildMarketPackage(input): MarketPackage
2. buildInterceptorContext(input): InterceptorContext
   ★ 必须传**全部**守护持仓（不是只传本标的）—— 只传本标的会让
     「反向持仓强制地板」在其它标的上形同虚设。
3. precheckTrade(input): PrecheckResult         // 纯函数，只编排既有模块
   precheckLiveTrade(req, deps): Promise<…>     // 活体外壳，依赖注入，不藏单例

★ 裁决是**四态**，互不顶替（判据 25）：
  pass                —— 全部通过，可以提交
  blocked             —— 不合格：去改报价 / 改参数
  approval_required   —— 需人工审批：去找人（blockers 必须为空）
  unverifiable        —— 查不了：去修数据源（UNVERIFIABLE_CODES 优先于 blocked）
  ★ 把 unverifiable 合并进 blocked，会让人去改一个本来没问题的止盈价 ——
    两者指向**相反的动作**。

★ 三条口径（都是实测补出来的）：
  ① pipeline 报 {checked, total}，而 total 是**全部闸门数**：
     查不了时报 `0/9`，不许报 `0/0` —— 后者读起来像「没有闸门」。
  ② cost / geometry 未评估时给 **null**，不给零值对象（判据 24）。
  ③ 敞口上限要区分 **null（未设置）** 与 **0（上限就是 0）**。
  ④ 活体要带 assumptions（我替你补了哪些数）与 disclosures（覆盖不到什么）。

★ 端点是**只读**的：POST /orders/precheck 不改状态、不占台账、不写审批单，
  但 appendEvent('ORDER_PRECHECK', …) 留痕可核 ——「只读」不等于「不留痕」。

★ 前端（交易大厅）：提交按钮由裁决驱动，三个条件缺一不可 ——
  有裁决 ∧ 裁决说 pass ∧ 裁决**没过期**。
  参数一改，旧裁决立刻标成过期（gateStale）并重新变灰；
  否则就是「改完参数拿旧绿放行」= 用过期结论给一笔没检查过的单背书。
  建议价来自引擎几何（computeStopGeometry / deriveStructureTarget），
  **不许在大厅硬编码 ±3%** —— 同一个事实两份口径，改一处必漏另一处。
```

---

## L6.25 · 横截面（breadth）：把「择时」换成「排序」

> 对应 `DEV_PROGRESS §3.35`。适用场景：单标的择时量出来的「每笔毛边际 vs 每笔成本」
> 差一个数量级，想换掉问题本身。

### L6.25.1 一句话定位

不再问「这只票接下来涨还是跌」，而是问「这一批标的里谁比谁强」。
两个目的同时达到：**市场方向被多空对冲消掉**、**每个时刻的样本从 1 个变成 N 个**。

### L6.25.2 五条必须照抄的设计决定

1. 面板对齐取**交集**，不补零、不前向填充 —— 补出来的 bar 会把「这个标的当时没交易」
   变成「它当时价格没动」，而后者是一个**真实的观测**，会直接进 IC 计算。
2. 因子在**原始** K 线上算、再投影到面板时间轴 —— 预热用的那几根不能因为
   「别的标的当时没有」就被砍掉，否则面板最前面几行的因子全是 null，
   而 null 会被下游读成「没有信号」而不是「没算」。
3. **方向由训练段定、判决只看检验段**。同一批数据上既定方向又评表现，
   反向的毛为正几乎是恒等式（`毛_rev ≈ −毛`），那不是证据。
   这一条对齐策略层的 `determineSign`：训练集定方向、样本外验证。
4. **绝不回落合成数据**。10 个合成品种之间没有真实相关性，
   在它们上面算出来的「横截面 IC」是一个看着完全正常的数字，而它测的是随机数生成器。
5. 毛与成本**同分母**：`每腿成本 = 单边成本 × 换手`，`换手 = Σ|Δside| / 腿数`。
   毛/成本比是唯一能分辨「该换因子族」与「该降换手」的那一对量。

### L6.25.3 ⚠️ 一个必须提前知道的陷阱（不知道会得出相反的结论）

**毛没有上界，成本有上界**：

```
每腿成本 = (费 + 滑) × 换手        换手 ≤ 2     ⇒ 成本 ≤ 2 × 8 = 16 bps
每腿毛   = mean(side · r)          r 随持有期增长 ⇒ 随持有期近似线性、无上界
```

⇒ 只要持有期足够长，「毛 > 成本」是**算出来的必然**，与「有没有 alpha」毫无关系。
实测（10 品种 × 34,920 根共同 bar）：1 小时档差 40 倍 → 12 小时档首次出现相交 →
2 天档 11/142 条「通过」。但把训练段占比从 0.6 挪到 0.7，2 天档的通过数**从 7 掉到 0**，
且**没有任何因子能在 3 个切分上都通过**（判据 6）。

⇒ **判据**：看到「长持有期通过」时先问一句 ——
「这是 alpha，还是**成本被持有期摊薄**了？」
这两种事因在「毛 > 成本」这一个数字上长得**一模一样**（判据 25）。

### L6.25.4 三态判决（与因子线一致，但三种 rejected 各有各的 gate）

| 结论 | 条件 | gate |
|---|---|---|
| accepted | 净 > 0 且胜率达标 | pass |
| rejected | ｜t｜ < 2 ⇒ 该换因子族 | ic-t |
| rejected | 毛 ≤ 0 ⇒ 方向反了 / 信号没用 | gross |
| rejected | 毛 < 成本 ⇒ 该降换手、该降费率 | cost |
| unverifiable | 横截面次数不够 / IC 算不出 t | sections / ic-unknown |
| unverifiable | 毛 ≥ 成本但胜率不够 / 净贴在 0 上 | winrate / net |
| unverifiable | 面板凑不出横截面 | panel |

★ 三种 rejected **必须有三个不同的 gate**：它们指向的动作完全相反
（换因子族 / 把排序反过来 / 降换手降费率）。共用一个词，台账里的
「被拒 109 条」就成了一句不可行动的话 —— 这是判据 20 的镜像。

### L6.25.5 烟测要钉的九类不变量（42 条断言）

```
交集不是并集（A1~A5）              · 池化 IC 是横截面不是时序（B1~B4）
毛与成本同分母（C1~C8）            · 三态互不顶替 + 三种 rejected 各有 gate（D1~D7）
训练段定方向 / 检验段判决（E1~E3）  · 面板不足落 panel 闸门、缺品种不回落（E4~E8）
成本口径唯一 / 不许合成 / 候选来自台账 / 没有第二份回测（F1~F9）
```

★ 变异验证 **6 处**（`_mutate_breadth.mjs`，**刻意保留** —— 它是「这些断言有牙」的
唯一可重现证据）：交集→并集退化、成本归零、三种 rejected 共用 gate、忽略 sign、
训练/检验合并、null 退化成 0 —— 全部当场变红，且每处只红 1~3 条
（说明断言之间没有互相兜底）。

### L6.25.6 复现这一层的提示词（可直接粘贴）

```
给现有量化系统加一条「横截面（breadth）」产线，要求：
1. 读多品种真实历史并对齐成**交集**面板：绝不补零、绝不前向填充、绝不回落合成数据。
   缺品种要指名报出来，不许静默剔除。
2. 因子在原始 K 线上算、再投影到面板时间轴（预热不许被砍）。
3. 候选因子只来自现有因子台账里 accepted 的行，不自己造候选。
4. 成本口径复用现有的 DEFAULT_EXEC，不新增任何费率字面量。
5. 方向由**训练段 IC 的符号**决定，判决**只看检验段**；
   并在通过时补一次反向检验（两向都过 ⇒ 这道门对方向没有区分力）。
6. 判决三态，三种 rejected 各用各的 gate 名。
7. 落台账时记**面板指纹**（品种集 + bar 数 + 各品种内容指纹 + topK/horizon/成本配置）：
   指纹变了，旧结论自动作废。
8. 写一个只测语义不变量的烟测并进 CI；对每个机制做变异验证，
   确认打坏它真的会变红（否则那条断言是假绿）。
9. 实跑时必须扫**多个持有期**并做**切分敏感性** ——
   「长持有期通过」很可能是成本被摊薄，不是 alpha。
```

### L6.25.7 本层的明确不做（V1）

- 不碰模型（排序、收益、成本全是确定性算术）；
- 不碰真实下单链路（只产出证据与判决，进不进池由既有闸门决定）；
- 不做 PBO 过拟合门（**待裁决**：是否把 `overfit.ts` 接进来）；
- 不扩标的池（当前 10 个；**待裁决** —— 每个横截面的 IC 只由 10 个点算秩相关）。

---

## L6.26 · 走势预测：把「能不能预测」变成**可证伪**的问题

> 对应 `DEV_PROGRESS §3.36`。适用场景：用户（或产品需求）要求"预测涨跌、给目标价位"。

### L6.26.1 先划清边界：不承诺"完美预测"，而且理由要是**算出来的**

需求原话常常是"要有完美的预测能力"。**不要答应，也不要含糊过去** ——
正确的回答是把"为什么不能"变成几条可核验的数字，而这件事本仓库已经算完了：
`§3.35` 证明长持有期的「毛 ≥ 成本」是成本被摊薄的结构性必然；切分敏感性检验证明
没有任何因子能在 3 个切分上都通过；而**最短那一档（未来 1 小时）的毛/腿天花板
0.61 bps 连成本地板 0.85 bps 都够不到**。

⇒ 于是模块的定位是：**永远给得出判断，并且永远同时给出这个判断值不值得信。**

### L6.26.2 五条必须照抄的设计决定

1. **永远不给单点预测。** 给的是条件分布：方向 = 中位收益的符号、目标 = 中位收益对应价位、
   走势图 = 10/50/90 分位带。单点不可证伪 ⇒ 不可校准 ⇒ 会变成永远吵不完的争论。
2. **基准必须是一个具体的平凡规则，并且写出来。** 本层用"永远猜**训练段**偏的方向"。
   ★ 基准规则**不许**用"整段里占多数的方向" —— 那个多数里含锚点之后的行情，
   等于**把结论建在泄漏上**，而且它会让结论偏向"没优势"，看着保守、其实更坏。
3. **比的是逐锚点配对差，不是两个独立比例。** 两个预测器用同一批锚点，
   所以标准误要从 `d_i = 命中_i − 基准_i` 估。用独立比例的 SE 会把噪声算错，
   而"算小"会平白判出优势 —— 这是这一层最贵的假绿。
4. **样本之间不许重叠。** 相隔不到 h 根的锚点共享同一段未来行情。
   当成独立样本 ⇒ 有效样本量虚高 ⇒ 区间被压窄 ⇒ **覆盖率检验看起来通过**。
   做法：按距离贪心取，已选点 ±h 之内全部跳过。
5. **`unverifiable` 时一个数字都不给**（方向/目标/区间/走势图全 `null`/空），
   而且**两条路径行为必须一致**（"数据太短"与"样本不够"）。
   `no-edge` 不一样：那里有证据说"这个方向没有优势"，数字留着是**带标注的信息**。

### L6.26.3 三态判决与 gate 名（每一种事因一个词）

| 结论 | 条件 | gate |
|---|---|---|
| actionable | 命中率显著优于基准 ∧ 覆盖率达标 ∧ 中位幅度 > 往返成本 | pass |
| no-edge | 配对差 z < 门槛 ⇒ **该换方法或攒数据** | edge |
| no-edge | 名义 80% 区间实际覆盖偏太多 ⇒ 区间偏窄 | coverage |
| no-edge | 方向可能对但幅度不够付成本 ⇒ **该降换手/降费率** | cost |
| unverifiable | 近邻数不够 | sample |
| unverifiable | 样本外锚点不够 ⇒ **判不了它准不准**（≠ 它不准） | calibration |
| unverifiable | 证据是合成/注入数据 | origin |

★ `edge` 与 `cost` 必须分开：前者指向"换方法"，后者指向"降成本"，
**动作完全相反**。共用一个词，台账里那句"没通过"就成了一句不可行动的话（判据 20）。

### L6.26.4 烟测要钉的不变量（21 条断言）+ 变异验证（11 处）

```
统计工具：空数组必须 null 不许退化成 0 · normalUpper 在已知点上的值 · ★ rollingZ 无未来函数
★ 滚动统计必须与独立实现逐点一致（含 null 洞）
近邻取样：两两相隔 ≥ horizon · 不许跨过"未来已实现"的边界
三态判决：合成数据不给 actionable · 样本不足不给数字（两条路都要测）· 没优势要说出来
确定性 / 口径：同输入同输出 · 成本只来自 DEFAULT_EXEC · 特征只来自 factorSeries
★ loadEvidence 与因子台账各只被调用一次（数据路径唯一）
校准：锚点全在训练段之后 · 基准规则要写出来 · z 与命中率同源（配对差）
```

★★ **变异验证抓出了两处"假绿"，这两处比 bug 值钱**：
- 夹具走的**不是**断言名说的那条路（`C2` 用 200 根，实际走"近邻池为空"，
  而它声称在测"数据太短"）⇒ 把 `blank()` 打坏也照绿。**修法是给两条路各写一条断言。**
- 只删**一道**守卫不足以破坏不变量（循环体里还有第二道）⇒
  变异验的是"冗余还在"，不是"断言有牙"。**打坏机制要打到最后一层。**

### L6.26.5 复现这一层的提示词（可直接粘贴）

```
给现有量化系统加一条"走势预测"产线，要求：
1. **只给分布，不给单点**：方向=中位收益符号，价位=中位收益对应价，走势图=10/50/90 分位带。
2. 用**历史相似状态**（analog）构造条件分布：状态向量取自现有因子台账里 accepted 的因子，
   按**训练段** |IC| 排序取前 K 个，用滚动 z（**只用过去**的窗口）标准化。
3. 特征必须复用现有引擎的 factorSeries，不许自己造；数据必须走现有证据层（含 origin/dataHash/gaps）。
4. 校准用 **walk-forward 样本外**：锚点全部落在训练段之后；报"方向命中率 / 对照的平凡规则命中率 /
   逐锚点配对差 z / 区间覆盖率 vs 名义覆盖"。
5. 平凡规则必须**无泄漏**（用训练段偏的方向，不许用整段的多数方向）。
6. 近邻之间不许重叠（相隔 < h 的跳过），否则有效样本量虚高、区间被压窄、覆盖率检验假绿。
7. 三态判决，且 **没有优势时如实说没有优势**：`no-edge` 是一个正式输出，不是一个含糊的"震荡"。
8. `unverifiable` 时**一个数字都不给**，且两条路径（数据太短 / 样本不够）行为一致。
9. 预测**只读**：不落盘、不下单。要不要下单交给**既有的**交易闸门 —— 一条业务动作只许一条路径。
10. 写只测语义不变量的烟测并进 CI；对每个机制做**变异验证**，
    并且**先检查变异本身**：锚点是否还命中、是不是打在了冗余守卫上、夹具是否真走了那条路。
```

### L6.26.6 本层的明确不做（V1）

- 不接 LLM 写解释（先由观测量生成，每条指向一个可核验字段）—— **待裁决**；
- 不扫状态向量维数 / 不扫 horizon（每多扫一档就多一次多重检验）—— **待裁决**；
- 不碰下单链路、不碰模型权重；不改任何状态。

---

## L6.27 · 让桌宠真的**按到界面上的按钮**，而且要能带上参数

> 对应 `DEV_PROGRESS §3.39`。适用场景：产品要求「我说一句话，界面就该动起来」，例如
> 「帮我预测比特币未来1小时的走势图」——**它得顺手把那张图调出来**。

### L6.27.1 一句话说清这一层在治什么

用户的原话是「**桌宠说到可以做到**」。要让它成立，必须回答两个不同的问题，而它们常被混成一个：

1. **它真的按到了那颗按钮吗？**（跨进程：服务端排队 → 浏览器执行）
2. **它按的那一下，和它嘴里说的那一次，是同一件事吗？**

第 2 个问题最容易被漏掉，因为漏掉之后**屏幕上一切正常**：它念「比特币未来 1 小时」，
界面按下后画的是**屏幕当前选着的**那个标的与尺度 —— 两个数各自都对，放在一起没有意义。
本仓库给这一类起了个编号（判据 31：**口径不同的两个数不许放在一起看**）。

### L6.27.2 九条必须照抄的设计决定

1. **服务端排队、界面执行、界面回报**。服务端按不到 DOM 里的按钮 ——
   它只能排一条有 id 的记录，由**界面自己**轮询后在**同一个 DOM 元素、同一个 onClick** 上按下去。
   这样就没有"桌宠专用的旁路"，也就没有绕开界面上任何一道确认的可能。
2. **队列是 append-only 文件，状态由最后一条记录推出**。重启不丢、进程无关、
   且能回答"是谁让它按的、按了之后成没成"。
3. **取活即认领**（服务端在一个函数里做"读 + 认领"）。多窗口同时开着时，
   只有第一个拿到；认领会过期，宁可重复执行可幂等的动作，也不要被一个死窗口卡住队列。
4. ★★ **参数必须跟着 `claim` 记录一起走**。任务视图由**最后一条**记录拼出，
   只把参数写在 `enqueue` 上，认领那一刻它就静默消失 —— 表现是"按了、但尺度是屏幕上原本那个"。
5. ★★ **参数走一次性同步通道，不走 Store**。按钮的 `onClick` 闭包里装着**上一次渲染**的 state，
   而 React 的 setState 是异步的 ⇒ "先 dispatch 再点击"读到的是旧值。所以：
   执行器在**点击之前**同步写入 → 页面在 onClick 里**取走即清空**。
   两道额外保护防两件不同的事：**actionId 不匹配不许取走**（取走＝那条参数永远到不了目的地）、
   **执行器在失败路径上显式撤回 + 短 TTL**（防"上一轮的参数留到下一次人手点击"）。
6. ★★ **参数在服务端就校验并归一，界面只负责照着画**。"这个标的存不存在"要行情注册表，
   "这个时长有没有证据底座"要预测层 —— 界面没有立场判断这两件事。
   不校验的后果不是"报错"，而是**静默回落成合成数据**，返回的每个字段都像真的（判据 13）。
7. **回话要说"真的发生了什么"**：等一个"某个窗口真的领走并回报"的三态
   （成功 / 界面报错 / 没人领），三种话指向三个**相反**的动作。
   全说成"调出来了"的后果是屏幕一片安静而账本里真有条记录 —— 看起来完全正常。
8. **人点与桌宠点走同一个函数**，差别只有"参数从哪来"；
   参数带过来时**先把屏幕也切过去**（标的 + 尺度），否则图上是 A、按钮上亮的是 B；
   档位控件要**含当前那个值**（排过来 30 分钟而常驻档是 15/60/240 ⇒ 三个按钮全不亮，
   屏幕上找不到任何一个 30，而它嘴里念的是 30）。
9. **只读的动作不需要两段式确认；会改状态的一律需要人确认过**（且那个 `confirmed`
   只能来自人的第二次点击）。两段式按钮**拒绝**代按 —— "上膛了但没跑"却回报成功，
   比直接失败更坑人。

### L6.27.3 ⚠️ 给这条通道写检查时，会连着踩到的三个坑

| 坑 | 形态 | 正解 |
| --- | --- | --- |
| 探针自己坏了 | 照抄另一个烟测的失败标记（`✗` vs `❌`）⇒ 所有变异报"0 条红"，结论全是错的 | 读**烟测自己打印的汇总行**，并交叉核对"汇总里的数字 = 后面列出的条数" |
| 断言没牙 | 断"某段字**出现过**" —— 把产生它的那一行改掉、消费端的字面量还在 ⇒ 照样绿 | 断**顺序/因果**（"不许出现在它之前"）或断**"只能有一处"**（调用点个数 = 1） |
| 一句话两个主人 | 同一句文案在面板与提交报错里各写一遍 ⇒ 删掉任一处都看不出来 | 收敛成**一处**（纯函数 + 唯一调用点）再断个数 |
| 变异把自己写歪 | 插进去的形状和断言的正则不是同一个排版 ⇒ "没抓到"其实是变异无效 | 变异**先自检**：锚点必须恰好命中 1 次；引号/排版要和断言口径一致 |

★ 每个变异跑完都必须**还原源码**，并在整轮结束时用 `md5sum -c` 逐文件核一遍
（"我改回来了"是个声明，不是一个观测）。

### L6.27.4 复现这一层的提示词（可直接粘贴）

```
按 L6.27 加一条"能带参数的界面动作"通道，要求：
1. 注册表里的动作可以声明它接受哪几个参数（白名单），并提供一个**归一函数**；
2. 入队时校验并归一，队列记录里存**归一后**的那一份，且 claim 记录必须把它带上；
3. 前端执行器在点击**之前**把参数写进一个模块级一次性通道，页面 onClick 取走即清空；
   参数 actionId 不匹配时不许取走；点击失败时显式撤回；通道带一个很短的 TTL；
4. 页面上人点与桌宠点走同一个函数，参数带过来时先把屏幕的标的与尺度也切过去；
5. 语音侧排完参数后**等"某个窗口真的回报"**，按 成功 / 界面报错 / 没人领 三态分别回话；
6. 给每一条都写断言（含真值表，不只断"字出现过"），并对每条做**变异验证**：
   变异必须让**对应那一条**变红，且跑完源码 md5 全部还原。
```

### L6.27.5 本层的明确不做（V1）

- 不给按钮加任意参数（只登记**有名字**的参数键，白名单之外一律拒收）；
- 不让桌宠代按**两段式**按钮，也不代按**"后果取决于面板里现在填着什么"**的按钮（`operatorOnly`）；
- 不改"从一句话里认标的"那条既有路径（`resolveUiAction`）—— 给它加参数会长出第二份口径，**待裁决**。
